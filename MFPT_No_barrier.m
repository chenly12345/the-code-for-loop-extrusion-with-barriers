% % theory soluation with no barrier

function T_MFPT = MFPT_No_barrier(a,b,c,n,TT)
%% Theoretical solution for the sliding of cohesin dimers on DNA lattice
% % This function is used to calculate the MFPT of different initial positions (no obstacles)
    ...
    if params.d==0
      ...
      d_vector = params.d*ones(size(P_t,1),1);
      d_vector(d_sigle(1:end-1)) = params.d/2;
      
      Fun_int = d_vector.*P_t; % Integrand function
      P_int   = trapz(T, Fun_int,2);
      P_off   = sum(P_int);

    % Boundary state transition rate: only two states can transition to the boundary, and the boundary point cannot transition out;
